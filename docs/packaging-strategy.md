...
vendoring must therefore be performed during update preparation. This is handled
automatically by `ci/prepare-update.sh` as part of preparing an upstream-update PR.
...
builds via `cargo build --release --locked --offline`. This is the approach used by the upstream
for its Launchpad PPA builds.

### `dh-cargo`/`debcargo` and `cdylib`

**Decision (recorded after review in issue #3):** The direct `cargo build --release --locked --offline` approach is the correct approach for NSS plugin modules and is aligned with accepted Debian practice.
...
2. **Direct `cargo build --release --locked --offline`** — for *plugin modules* with a fixed ABI convention (NSS, codec plugins, etc.) where there are no C headers or pkg-config files to generate. This is the accepted Debian pattern for NSS modules.
...
`vendor.tar.gz` is regenerated automatically by `ci/prepare-update.sh` whenever
a new upstream release is prepared. It contains:

- `.cargo/config.toml` — generated by `cargo vendor --locked`
- `vendor/` — the vendored crate sources

Vendor-local `Cargo.lock` files are deliberately excluded from `vendor.tar.gz`.
The repository-root `Cargo.lock` is the single authoritative lockfile; package
builds run `cargo --locked --offline` using that file.
...
`rustc`. The authoritative CI environment is Debian Trixie (`rustc 1.85.0`). Ubuntu
Noble's archive `cargo`/`rustc` 1.75.0 were too old for the current vendored tree
(unable to parse the upstream lockfile format v4 and some vendored `edition2024`
manifests) and were superseded by the Trixie-based CI container.
...
6. **`dh-cargo`/`cargo-c` revisit:** **Resolved in issue #3.** See "Debian cdylib
   packaging decision" section above.       `cargo build --release --locked --offline` is the
...
- `debian/rules` — `dh $@ --with installnss`, `RUSTFLAGS` SONAME injection,
  `cargo build --release --locked --offline`, versioned `.so.2` symlink
...
- `vendor.tar.gz` — offline build archive (`.cargo/config.toml` + `vendor/`, no Cargo.lock);
...
unit tests (`cargo test --locked --offline`) run during `dh_auto_test` and pass. `vendor.tar.gz`
